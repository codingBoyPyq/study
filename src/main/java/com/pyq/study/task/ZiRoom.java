...
public class ZiRoom {
    ...
    public static void moniter() throws Exception {
        while (true) {
            String result = "";
            ...
            System.out.println("time:" + sdf.format(System.currentTimeMillis()) + " status:" + status);
            if (!"可约看".equals(status)) {
                Thread alarmThread = new Thread(new Alarm());
                alarmThread.start();
                alarmThread.join();
            }
            Thread.sleep(500);
